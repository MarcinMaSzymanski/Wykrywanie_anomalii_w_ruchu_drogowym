...
def findAccelerations():
    query = "SELECT iRide FROM Rides WHERE processedState=\"Accelerations\""
    result = c.readQuery(query)
    return result

def setPreProcessed(ride):
    query = "UPDATE Rides Set processedState=\"PreProcessed\" Where iRide=" + str(ride)
    result = c.updateQuery(query)
